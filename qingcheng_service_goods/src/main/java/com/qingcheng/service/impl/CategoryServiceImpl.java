...
public class CategoryServiceImpl implements CategoryService {
    ...
    public void saveCategoryTreeToRedis() {
        System.out.println("加载商品分类到缓存中");
        Example example = new Example(Category.class);
        Example.Criteria criteria = example.createCriteria();
        criteria.andEqualTo("isShow",1);// 显示
        List<Category> categories = categoryMapper.selectByExample(example);
        List<Map> categoryTree = findByparentIds(categories, 0);
    ...
    public List<Map> findCategoryTree() {
      /*  Example example=new Example(Category.class);
        ...
        List<Category> categories = categoryMapper.selectByExample(example);
        return findByparentIds(categories,0);*/
      return (List<Map>) redisTemplate.boundValueOps(CacheKey.CATEGORY_TREE).get();
    }



    private List<Map> findByparentIds(List<Category> categories,Integer parentId){
        List<Map> list =new ArrayList<>();
        for (Category category : categories) {
            if (category.getParentId().equals(parentId)){
                Map map = new HashMap<>();
                map.put("name",category.getName());
                map.put("menus",findByparentIds(categories,category.getId()));
                list.add(map);
