...
public class SpuServiceImpl implements SpuService {
    ...
    public void saveGoods(Goods goods) {
        ...
        // 保存sku页面时间和对应id
        Map<String, Date> skuCreateTimeMap = new HashMap<>();
        if (spu.getId() == null) {//新增商品
            ...
            //在删除前保存sku的创建时间
            List<Sku> skuOldList = skuMapper.selectByExample(example);
            for (Sku sku : skuOldList) {
                String id = sku.getId();
                System.out.println();
                Date createTime = sku.getCreateTime();

                skuCreateTimeMap.put(id, createTime);
              /*  System.out.println(skuCreateTimeMap);*/
                skuService.savePriceToRedisById(sku.getId(),sku.getPrice());
            }

            //删除修改spu所对应的sku,方便之后的修改
        ...
        for (Sku sku : skuList) {
            ...
            if (sku.getId() == null) {//新增
               ...
               /* for (String s : skuCreateTimeMap.keySet()) {
                    // 如果id相等,直接赋值createTime的值
                    if (sku.getId().equals(s)) {
                        sku.setCreateTime(skuCreateTimeMap.get(s));
                    }
                }*/
                // 直接根据sku.id获取从map中获取键值对
                sku.setCreateTime(skuCreateTimeMap.get(sku.getId()));
            }
    ...
    public void audit(String id, String status, String message) {
        ...
        ck.setId(spu.getId());
        ck.setCheckMessage(message);
        ck.setCheckTime(new Date());
        ck.setCheckUser("admin");
        ck.setCheckEnd(Integer.parseInt(status));
        checkLogMapper.insert(ck);
